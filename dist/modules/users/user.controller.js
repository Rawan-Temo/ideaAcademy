...
const getAllUsers = async (req, res) => {
  try {
    const { rows, count } = await user_service_1.UserService.getAllUsers(
      req.query,
    );
    const reponse = {
      status: "success",
...
const login = async (req, res) => {
  try {
    ...
    if (!user) {
      return (0, response_1.sendBadRequest)(
        res,
        "Invalid Username or Password",
      );
    }
    let isPasswordValid = await bcrypt_1.default.compare(
      password,
      user.password,
    );
    if (!isPasswordValid) {
